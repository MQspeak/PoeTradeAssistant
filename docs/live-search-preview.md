...
更新日期：2026-09-07。基准分支：`main`；功能仍保留试用验证边界。
...
已实现离线翻译、单项监控操作、命中已读 UI 和独立运行状态；自定义声音及完整旧 state 迁移尚未实现。
...
- 登录验证通过后显示成功提示并隐藏本次由应用启动的浏览器，验证按钮切换为红色断开按钮。
...
应用本次启动的浏览器在登录验证后完全隐藏，启动监控及创建监控页时会再次确保窗口保持隐藏；使用“显示浏览器”恢复。连接已有实例时保留可见，避免隐藏用户其他标签页。主动断开和退出前恢复隐藏窗口。浏览器意外关闭时取消自动前往并清理会话；界面操作设有 60 秒超时，关闭清理最多等待约 7 秒，前往请求最多等待 5 秒。
