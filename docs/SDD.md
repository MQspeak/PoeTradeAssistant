...
> 2026-09-05 分支增量：`codex/live-search-chromium` 已新增原生实时搜索试用模块，使用随包完整 Chromium。实现、依赖和验证边界见 [实时搜索试用说明](live-search-preview.md)。下文 v1.1 描述扫描器与计算器基线；其中“单进程/无浏览器运行时”不适用于新增监控模块。监控报价尚不导入扫描数据合约或计算器。
